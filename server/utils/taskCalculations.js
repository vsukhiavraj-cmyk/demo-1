...
export const calculateTaskRequirements = (months, userProfile = {}) => {
    ...
    const timeCommitment = Math.max(1, Math.min(5, Number(userProfile.timeCommitment) || 3));
    const experienceLevel = Math.max(1, Math.min(5, Number(userProfile.experienceLevel) || 2));
    const consistency = Math.max(1, Math.min(5, Number(userProfile.consistency) || Number(userProfile.focusCapability) || 3));
    
    // Validate that we have valid numbers
    if (isNaN(timeCommitment) || isNaN(experienceLevel) || isNaN(consistency)) {
        console.error('Invalid user profile values:', { timeCommitment, experienceLevel, consistency, userProfile });
    ...
    const taskComplexity = calculateTaskComplexity(timeCommitment, experienceLevel);
    
    // Determine optimal task count using hybrid approach (80% baseline + capacity adjustment + realistic completion check)
    const finalTaskCount = calculateOptimalTaskCount(totalDays, effectiveStudyDays, dailyHours, taskComplexity, daysPerWeek, experienceLevel, consistency);
    ...
    const avgHoursPerTask = getBaseTaskHours(dailyHours, daysPerWeek, taskComplexity);

    // Calculate capacity-based tasks for comparison (before realistic capping)
    const baseTaskHours = getBaseTaskHours(dailyHours, daysPerWeek, taskComplexity);
    ...
    return {
        ...
        capacityUtilization: Math.round((finalTasksPerWeek * avgHoursPerTask / weeklyHours) * 100),
        // New realistic completion info
        realisticMaxTasks: Math.round(effectiveStudyDays * Math.min(1.5, ((experienceLevel + consistency) / 10) * 2)),
        realismFactor: Math.round(((experienceLevel + consistency) / 10) * 100) / 100,
 ...
 */
const calculateOptimalTaskCount = (totalDays, effectiveStudyDays, dailyHours, taskComplexity, daysPerWeek, experienceLevel, consistency) => {
    // Parameter validation to prevent undefined errors
    if (typeof experienceLevel !== 'number' || typeof consistency !== 'number') {
        console.error('Invalid parameters in calculateOptimalTaskCount:', { experienceLevel, consistency });
        throw new Error(`Invalid parameters: experienceLevel=${experienceLevel}, consistency=${consistency}`);
    }
    ...
    const realisticMaxTasks = Math.round(effectiveStudyDays * maxTasksPerStudyDay);
    ...
    const finalTaskCount = Math.max(minReasonableTasks, Math.min(optimalTaskCount, maxReasonableTasks));
    ...
    - Realism Factor: ${realismFactor.toFixed(2)} → Adjusted: ${adjustedRealismFactor.toFixed(2)}
...
const getBaseTaskHours = (dailyHours, daysPerWeek, taskComplexity) => {
    ...
    if (weeklyHours >= 35) { // High capacity (35+ hours/week)
        baseHours = taskComplexity.level === "Expert" ? 4 : 
                   taskComplexity.level === "Advanced" ? 3 : 
                   taskComplexity.level === "Intermediate" ? 2.5 : 2;
    } else if (weeklyHours >= 20) { // Medium capacity (20-34 hours/week)
        baseHours = taskComplexity.level === "Expert" ? 3 : 
                   taskComplexity.level === "Advanced" ? 2.5 : 
                   taskComplexity.level === "Intermediate" ? 2 : 1.5;
    } else if (weeklyHours >= 10) { // Low capacity (10-19 hours/week)
        baseHours = taskComplexity.level === "Expert" ? 2.5 : 
                   taskComplexity.level === "Advanced" ? 2 : 
                   taskComplexity.level === "Intermediate" ? 1.5 : 1;
    } else { // Very low capacity (< 10 hours/week)
        baseHours = taskComplexity.level === "Expert" ? 2 : 
                   taskComplexity.level === "Advanced" ? 1.5 : 
                   taskComplexity.level === "Intermediate" ? 1 : 0.5;
    }
 ...
 */
export const validateTaskCount = (generatedTaskCount, months, userProfile = {}) => {
    const requirements = calculateTaskRequirements(months, userProfile);
...
export const printTaskCountBreakdown = (userProfile = {}) => {
    ...
    console.log(`User Profile: Time Commitment ${timeCommitment}/5 (${dailyHours}h/day), Experience Level ${experienceLevel}/5`);
    console.log("\n📊 OPTIMIZED TASK COUNT BREAKDOWN:");
    console.log("Duration | Tasks | Hrs/Task | Task Type | Complexity | Utilization");
    console.log("---------|-------|----------|-----------|------------|------------");

    const durations = getSupportedDurations(userProfile);
    durations.forEach(({ label, minTasks, avgHoursPerTask, taskType, taskComplexity, capacityUtilization }) => {
        console.log(`${label.padEnd(8)} | ${minTasks.toString().padEnd(5)} | ${avgHoursPerTask.toString().padEnd(8)} | ${taskType.padEnd(9)} | ${taskComplexity.padEnd(10)} | ${capacityUtilization}%`);
    });

    console.log("\n⚡ SMART TASK APPROACH:");
    durations.forEach(({ label, minTasks, taskComplexity, avgHoursPerTask }) => {
        console.log(`${label}: ${minTasks} ${taskComplexity.toLowerCase()} tasks (${avgHoursPerTask}h each)`);
    });

    console.log("\n✅ INTELLIGENT TASK SIZING:");
    console.log("• Low commitment users: More frequent, guided tasks (1-2h each)");
